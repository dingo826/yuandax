function J_Uploader(){
    ...
    J_Uploader.error      = {'-1':"文件格式不对",'-2':"网络错误",'-3':'目录不存在','-4': '未知错误'};
        ...
        if(J_Uploader.list[J_Uploader.now].filezise > J_Uploader.filesize){
          ...
          J_Uploader.finalupload();
        }
			}
		}
		
    J_Uploader.finalupload=function(){
      //上传核心
      if(J_Uploader.process!=false){
        J_Uploader.processupload();
      }else{
...
J_Uploader.compress = function() {
  ...
  img.onload=function(){
    ...
    J_Uploader.list[J_Uploader.now].data = canvas.toDataURL('image/jpeg', J_Uploader.quality);
    moreCanvas.width = moreCanvas.height = canvas.width = canvas.height = 0;
    J_Uploader.finalupload();
  }
  
  J_Uploader.list[J_Uploader.now].filetype='jpg';
  img.src=J_Uploader.list[J_Uploader.now].data;
};

//上传回调进度
J_Uploader.processupload=function(){
  var xhr = new XMLHttpRequest();
  xhr.open('post', J_Uploader.upload_url);
  xhr.onload = function(e) {
    if ((xhr.status >= 200 && xhr.status < 300) || xhr.status === 0) {
      var backdata = JSON.parse(xhr.responseText);
      J_Uploader.finish(backdata);
    }
    else {
      J_Uploader.finish({status:-2});
    }
  }
  
  xhr.upload.addEventListener('progress', function(e) {
    pecent = ~~(100 * e.loaded / e.total);
    J_Uploader.process(pecent);
    //span.style['width'] = pecent + '%';
    //span.innerHTML = (pecent == 100 ? 99 : pecent) + '%';
  }, false);
  
  var data='data='+encodeURIComponent(J_Uploader.list[J_Uploader.now].data)+'&filename='+encodeURIComponent(J_Uploader.list[J_Uploader.now].filename)+'&filetype='+encodeURIComponent(J_Uploader.list[J_Uploader.now].filetype);
