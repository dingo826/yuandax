	...
	function checklogin() {
		...
		if($weixin['packid']!=2) {
			if($weixin['etime']<time()) {
				echo "<script>alert('该试用账号已到期，请联系客服。');location.href='".U('user/index/index')."';</script>";
			}
		}

		if($weixin['sysversion']==2) {
			header("location: ".U("weixinv3/editing/index"));exit;
		}
