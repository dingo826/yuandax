<?php
class ArticleAction extends Weixinv3Action {
	...
	public function add() {
		...
		if($_POST) {
			...
			header("Location: ".U("category/zilei?id=".$post['cid']));
      exit;
		}
		$catlist = $this->_cateModel->getCategoryList($wid, "article");

    $detail['cid'] = I("get.cid");
    ...
    //echo"<script>history.go(-1);</script>";  
		//header("Location: ".U("weixinv3/category/zilei?id=&cid".));exit;
    header('location: '.$_SERVER['HTTP_REFERER']);exit;//获取源地址并刷新
	}
  ...
  public function orderadd() {
		...
		if($_POST) {
			...
			M("app_article")->add($data);
			header("Location: ".U("article/index?id=".$post['cid']));exit;
		}
		$catlist = $this->_cateModel->getCategoryList($wid, "article");
